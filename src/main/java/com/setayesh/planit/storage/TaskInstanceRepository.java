...
public class TaskInstanceRepository {
    ...
    private void initSchema() {
        try (Connection conn = DriverManager.getConnection(url, USER, PASSWORD);
            ...
            stmt.execute("""
                        ...
                        ON task_instances_completed(task_id);
                    """);
    ...
    /**
     * Check if a completed instance exists.
     */
    public boolean exists(UUID taskId, LocalDate date) {
        return isCompletedOnDate(taskId, date);
    }

    /**
     * Return all completed dates for a task.
     */
    ...
    /**
     * Delete all completed occurrences AFTER a given date.
     * Needed for /delete-future/{date}.
     */
    public void deleteFutureInstances(UUID taskId, LocalDate fromDate) {
        String sql = """
                    DELETE FROM task_instances_completed
        ...
        try (Connection conn = DriverManager.getConnection(url, USER, PASSWORD);
            ...
            System.err.println("⚠️ Error deleting future task instances: " + e.getMessage());
        }
    }
}
